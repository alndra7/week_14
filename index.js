...
directors.forEach((item) => {
  const directorName = document.createElement("h3");
   ...
   const career = document.createElement("div");
    ...
    const filmLink = document.createElement("p");
    filmLink.innerHTML = `<a href="${item.films}">фильмография</a>`;
    container.append(filmLink);
  });
  
const topFilmsList = [
    directors.forEach((item) => {
     const topFilms = document.createElement("span");
     topFilms.textContent = `${item.top_rated_film}, `;
     bestFilms.append(topFilms);
    })
  ];
